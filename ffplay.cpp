...
int MediaPlayer::stream_component_open(VideoState *is, int stream_index)
{
    ...
    switch (avctx->codec_type) {
...
#if CONFIG_AVFILTER
        {
            ...
            if ((ret = decoder_start(&is->auddec, audio_thread)) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if ((ret = decoder_start(&is->viddec, video_thread)) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            if ((ret = decoder_start(&is->subdec, subtitle_thread)) < 0)
                goto out;
            break;
        default:
            break;
...
int MediaPlayer::audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
...
int MediaPlayer::video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            if ((ret = configure_video_filters(graph, is, player->get_vfilters_list() ? player->get_vfilters_list()[is->vfilter_idx] : NULL, frame)) < 0) {
                SDL_Event event;
...
VideoState * MediaPlayer::stream_open(const char *filename, AVInputFormat *iformat)
{
...
int MediaPlayer::realloc_texture(SDL_Texture **texture, Uint32 new_format, int new_width, int new_height, SDL_BlendMode blendmode, int init_texture)
{
    Uint32 format;
    int access, w, h;
...
int MediaPlayer::decoder_start(Decoder *d, int (*fn)(void *))
{
...
void MediaPlayer::set_filename(char * filename) {
    media_filename = filename;
    
    is = stream_open(media_filename, NULL);
    if (!is) {
...
int MediaPlayer::get_video_frame(VideoState * state, AVFrame *frame)
{
    ...
    if (got_picture) {
        ...
        frame->sample_aspect_ratio = av_guess_sample_aspect_ratio(state->ic, is->video_st, frame);
...
int MediaPlayer::upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    int ret = 0;
    Uint32 sdl_pix_fmt;
...
void MediaPlayer::fill_rectangle(int x, int y, int w, int h)
{
    SDL_Rect rect;
    rect.x = x;
    rect.y = y;
...
int MediaPlayer::configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame)
{
...
void MediaPlayer::video_refresh(double *remaining_time)
{
    ...
    display:
        /* display picture */
        if (is->force_refresh && is->show_mode == VideoState::SHOW_MODE_VIDEO && is->pictq.rindex_shown)
            video_display(is);
    }
    is->force_refresh = 0;
}
